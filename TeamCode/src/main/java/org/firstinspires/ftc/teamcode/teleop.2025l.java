...
public class Tutor_mecanum_gobilda_autoSequentailOrManual extends LinearOpMode {
    ...
    private final String LIGHT_CONFIG_NAME="light";


    private final double X_OFFSET_TO_Y_DEADWHEEL_MM = 80.0; //see gobilda pinpoint user manual
    private final double Y_OFFSET_TO_X_DEADWHEEL_MM = -384.0; //see gobilda pinpoint user manual

    //Starting point in field coordinates
    ...
    // +Y is points to the left and is 90 deg
    private final double START_X_VALUE = 0.0;   //17.0 * 25.4 from edge of arena to front of robot (in mm);
    private final double START_Y_VALUE = 0.0;
    private final double START_HEADING_VALUE = ODOMETRY_ANGLE_UNIT.fromDegrees(0.0);


    // --- PID Constants ---
    private final double P_DRIVE_COEFF = 0.02;
    private final double I_DRIVE_COEFF = 0.000;
    ...
    private final double P_HEADING_COEFF = 0.60;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        currentState = RobotState.IDLE; // Start in IDLE, require user input to begin
    ...
    private void runAutonomousSequence() {
        ...
        switch (autonomousSequenceStep) {
            ...
            case 0:
                telemetry.addLine("Sequence: Step 1 (200, 0.0, 0deg)");
                navigateToTargetWaypoint(new Waypoint(-100.0, 0.0, ODOMETRY_ANGLE_UNIT.fromDegrees(180), true));
                if (opModeIsActive()) autonomousSequenceStep++; // Move to next step if OpMode still active
                break;
            case 1:
                telemetry.addLine("Sequence: Step 2 (200, 200, 90deg)");
                navigateToTargetWaypoint(new Waypoint(-100, 120, ODOMETRY_ANGLE_UNIT.fromDegrees(180), true));
                if (opModeIsActive()) autonomousSequenceStep++;
                break;
            case 2:
                telemetry.addLine("Sequence: Step 3 (0.0, 200, 0deg)");
                navigateToTargetWaypoint(new Waypoint(0.0, 1200, ODOMETRY_ANGLE_UNIT.fromDegrees(180), false));
                if (opModeIsActive()) autonomousSequenceStep++;
                break;
            case 3:
                telemetry.addLine("Sequence: Step 4 (0, 0, 270deg)");
                navigateToTargetWaypoint(new Waypoint(0.0, 0.0, ODOMETRY_ANGLE_UNIT.fromDegrees(180), true));
                if (opModeIsActive()) autonomousSequenceStep++;
                break;
            case 4:
                //telemetry.addLine("Sequence: Step 5 (0.0, 0.0, 0deg)");
                //navigateToTargetWaypoint(new Waypoint(0.0, 0.0, ODOMETRY_ANGLE_UNIT.fromDegrees(0), true));
                if (opModeIsActive()) autonomousSequenceStep++;
                break;
            default:
    ...
    private void initializeDriveMotors() {
        ...
        light = hardwareMap.get(CRServo.class, LIGHT_CONFIG_NAME);
        ...
        frontLeft.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);        frontRight.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
    ...
    private void handleManualDrive() {
        ...
        double rxInput = gamepad1.right_stick_x;// Rotational input from joystickshooterRight.setPower(.53);
        if(gamepad1.x) {
            shooterRight.setPower(-.48);
            shooterLeft.setPower(.48);
            light.setPower(.01);
        }
        if (gamepad1.y) {
